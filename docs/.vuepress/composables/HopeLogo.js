...
export default defineComponent({
    ...
    setup() {
        ...
        const sizes = computed(() => isMobile.value ? { width: 220, height: 220 } : { width: 300, height: 300 });
        const renderLogo = async (three, STLLoaderConstructor, OrbitControlsConstructor) => {
            const { width, height } = sizes.value;
            // Canvas
            const canvas = document.querySelector("canvas#hero-logo");
            // Scene
            ...
            let logo2;
            // Lights
            const ambientLight = new three.AmbientLight(0xffffff, isDarkmode.value ? 5 : 15);
            ...
            tick();
            await Promise.all([
                new Promise((resolve) => stlLoader.load(`${ASSETS_SERVER}/model/logo1.stl`, (geometry) => {
                ...
                new Promise((resolve) => stlLoader.load(`${ASSETS_SERVER}/model/logo2.stl`, (geometry) => {
                    const material = new three.MeshPhysicalMaterial({
                        color: 0x35495e,
                        metalness: 0.7,
                        roughness: 0.5,
        ...
        onMounted(() => {
            watch([isDarkmode, isMobile], () => Promise.all([
                import("three").then((m) => m),
                import("three/examples/jsm/controls/OrbitControls.js").then((m) => m),
                import("three/examples/jsm/loaders/STLLoader.js").then((m) => m),
            ]).then(([THREE, { OrbitControls }, { STLLoader }]) => renderLogo(THREE, STLLoader, OrbitControls)), { immediate: true });
        });
        return () => [
            ready.value
                ? null
