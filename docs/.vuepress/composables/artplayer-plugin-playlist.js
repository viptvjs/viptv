const artplayerPlaylist = (options) => (art) => {
  ...
  // 自动播放下一P
  const currentEp = options.playlist.findIndex(
    (videoInfo) => videoInfo.url === art.option.url
  );
  ...
  const icon =
    '<i class="art-icon"><svg class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" width="22" height="22"><path d="M810.666667 384H85.333333v85.333333h725.333334V384z m0-170.666667H85.333333v85.333334h725.333334v-85.333334zM85.333333 640h554.666667v-85.333333H85.333333v85.333333z m640-85.333333v256l213.333334-128-213.333334-128z" fill="#ffffff"></path></svg></i>';
  ...
  art.controls.add({
    ...
    selector: options.playlist.map((videoInfo, index) => ({
      html: `${index + 1}. ${videoInfo.title || `NO.${index + 1}`}`,
