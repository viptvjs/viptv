...
<script setup>
  ...
  onMounted(async () => {
    state.value.a = (await vod.find({ "name": "lzzy-16" })).data
    state.value.b = (await vod.find({ "name": "bfzy-32" })).data
    state.value.c = (await vod.find({ "name": "yzzy-15" })).data
    a(0)
  });
  const a = (key) => {
    const { a } = state.value
