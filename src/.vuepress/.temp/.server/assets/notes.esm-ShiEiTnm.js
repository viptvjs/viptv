...
class _ {
    ...
    if (e2) {
      ...
      if (t3.header.length === t3.align.length) {
        ...
        for (a2 = t3.header.length, i2 = 0; i2 < a2; i2++)
          t3.header[i2].tokens = this.lexer.inline(t3.header[i2].text);
        for (a2 = t3.rows.length, i2 = 0; i2 < a2; i2++)
          for (r2 = t3.rows[i2], s2 = 0; s2 < r2.length; s2++)
            r2[s2].tokens = this.lexer.inline(r2[s2].text);
        return t3;
      }
    }
  }
  ...
  url(t2, e2) {
    ...
    if (n2 = this.rules.inline.url.exec(t2)) {
      let t3, i2;
      if ("@" === n2[2])
        t3 = l(this.options.mangle ? e2(n2[0]) : n2[0]), i2 = "mailto:" + t3;
      else {
        let e3;
        do {
...
class R {
  ...
  inlineTokens(t2, e2 = []) {
    ...
    for (; t2; )
      if (a2 || (o2 = ""), a2 = false, !(this.options.extensions && this.options.extensions.inline && this.options.extensions.inline.some((i3) => !!(n2 = i3.call({ lexer: this }, t2, e2)) && (t2 = t2.substring(n2.raw.length), e2.push(n2), true))))
        if (n2 = this.tokenizer.escape(t2))
          ...
          if (n2 = this.tokenizer.inlineText(s2, E))
            t2 = t2.substring(n2.raw.length), "_" !== n2.raw.slice(-1) && (o2 = n2.raw.slice(-1)), a2 = true, i2 = e2[e2.length - 1], i2 && "text" === i2.type ? (i2.raw += n2.raw, i2.text += n2.text) : e2.push(n2);
          else if (t2) {
            const e3 = "Infinite loop on byte: " + t2.charCodeAt(0);
            if (this.options.silent) {
              console.error(e3);
      ...
      if (d2 = t2[n2], this.options.extensions && this.options.extensions.renderers && this.options.extensions.renderers[d2.type] && (y2 = this.options.extensions.renderers[d2.type].call({ parser: this }, d2), false !== y2 || !["space", "hr", "heading", "code", "table", "blockquote", "list", "html", "paragraph", "text"].includes(d2.type)))
        ...
        switch (d2.type) {
          ...
          case "code":
            ...
            continue;
          case "table":
            for (c2 = "", l2 = "", r2 = d2.header.length, i2 = 0; i2 < r2; i2++)
              l2 += this.renderer.tablecell(this.parseInline(d2.header[i2].tokens), { header: true, align: d2.align[i2] });
            for (c2 += this.renderer.tablerow(l2), u2 = "", r2 = d2.rows.length, i2 = 0; i2 < r2; i2++) {
              for (o2 = d2.rows[i2], l2 = "", a2 = o2.length, s2 = 0; s2 < a2; s2++)
                l2 += this.renderer.tablecell(this.parseInline(o2[s2].tokens), { header: false, align: d2.align[s2] });
...
function P(t2, e2) {
  return (n2, i2, s2) => {
    ...
    if (function(t3) {
      ...
      if (!r3 || r3.length < 3)
        return l2();
      if (delete i2.highlight, !o2.length)
        return l2();
      let c2 = 0;
    ...
    if (i2.async)
      return Promise.resolve(i2.hooks ? i2.hooks.preprocess(n2) : n2).then((e3) => t2(e3, i2)).then((t3) => i2.walkTokens ? Promise.all(O.walkTokens(t3, i2.walkTokens)).then(() => t3) : t3).then((t3) => e2(t3, i2)).then((t3) => i2.hooks ? i2.hooks.postprocess(t3) : t3).catch(a2);
    try {
...
O.options = O.setOptions = function(t2) {
    ...
    if (t3.hooks) {
      ...
      for (const n3 in t3.hooks) {
        ...
        q.passThroughHooks.has(n3) ? e3[n3] = (s2) => {
          if (O.defaults.async)
            return Promise.resolve(t3.hooks[n3].call(e3, s2)).then((t4) => i2.call(e3, t4));
  ...
  let n2 = [];
  for (const i2 of t2)
    switch (n2 = n2.concat(e2.call(O, i2)), i2.type) {
      case "table":
        for (const t3 of i2.header)
          n2 = n2.concat(O.walkTokens(t3.tokens, e2));
        for (const t3 of i2.rows)
          for (const i3 of t3)
            n2 = n2.concat(O.walkTokens(i3.tokens, e2));
        break;
      case "list":
