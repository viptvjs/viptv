...
<SiteInfo v-for="(item,k) in state.vodsn" :name="item.title" :desc="item.vod_content" :logo="item.vod_pic"
  :preview="item.vod_pic" url="" @click="vodsnurl(k)" />
</template>
<template #tab1="{ value, isActive }">
<SiteInfo v-for="(item,k) in state.vodyl" :name="item.title" :desc="item.vod_content" :logo="item.vod_pic"
