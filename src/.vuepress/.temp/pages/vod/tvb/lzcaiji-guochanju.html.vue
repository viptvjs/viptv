<template><div><ArtPlayer :src="state.src" :config="artPlayerConfig" /><Tabs id="2" :data='[{"id":"量子资源"},{"id":"鱼乐资源"}]' :active="0">
<template #title0="{ value, isActive }">量子资源</template>
<template #title1="{ value, isActive }">鱼乐资源</template>
<template #tab0="{ value, isActive }">
<SiteInfo v-for="(item,k) in state.vodlz" :name="item.vod_name" :desc="item.vod_content" :logo="item.vod_pic"
  :preview="item.vod_pic" url="" @click="vodlzurl(k)" />
</template>
<template #tab1="{ value, isActive }">
<SiteInfo v-for="(item,k) in state.vodyl" :name="item.vod_name" :desc="item.vod_content" :logo="item.vod_pic"
...
<script setup lang="ts">
  ...
  onMounted(async () => {
    const lzcaiji = await vod.find({ "name": "lzcaiji-guochanju" })
