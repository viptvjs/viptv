...
import { existsSync, writeFileSync, writeFile, readFileSync, mkdirSync } from "node:fs";
...
function writeMap(info) {
    try {
        ...
        images.unshift(info);
        images.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime() // 按日期从大到小排序
        );
        ...
        if (!isExistsArchive) {
            ...
            archives.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime() // 按日期从大到小排序
            );
        }
...
async function writeReadme(images, archives) {
    try {
        // 展示最新的31条
...
function writeArchive(dateMonth, info, mapPath, previewPath) {
    try {
...
function writeDocs(info, images, archives) {
    // 最新的31条
    const imageList = images.slice(0, 31);
    const writeDataList = [];
    imageList.forEach((item) => {
        ...
        writeDataList.push(`[Download 1920 * 1080](${item.url_1080}) | [Download 3840 * 2160](${item.url_4k})\n\n`);
    });
    writeFile("../../docs/viptv/src/archives/README.md", writeDataList.join(""), (err) => {
        if (err) {
            throw err;
        }
    });
...
function writeDocsArchive(info, archives) {
    if (!archives.length) {
        return;
    }
    let targerArchive = null;
    ...
    const { date, mapPath } = targerArchive;
    const buffer = readFileSync(mapPath);
    const stringData = buffer.toString();
    const imageList = JSON.parse(stringData);
    const writeDataList = [];
    imageList.forEach((item) => {
        writeDataList.push(`## ${item.date} ${item.title}\n\n`);
        writeDataList.push(item.copyright + "\n\n");
...
function writeSidebar(archives) {
    const sidebar = [{ text: "最近更新 31", link: "/archives/" }];
    const archiveMap = {};
    archives.forEach((item) => {
    ...
    writeFile(`../../docs/viptv/src/.vuepress/config/sidebar/archives.ts`, sidebarDate, (err) => {
        if (err) {
            throw err;
        }
    });
