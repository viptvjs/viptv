...
<template>
  <div>
    <!-- 遍历 Aside_Data 数组并渲染 -->
    <a v-for="(banner, index) in Aside_Data" :key="index" :href="banner.link"
      :class="{ 'has-activity': banner.Activity, 'has-name': banner.name }" :title="banner.Activity || banner.name"
      class="banner" target="_blank" rel="noopener">
